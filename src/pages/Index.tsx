...
import { 
  ...
  Sparkles, Megaphone, Search, X, Nut, Package, Scale, Droplet, Mic, ChevronRight, Store, BookOpen, Globe
} from "lucide-react";
...
const Index = () => {
  ...
  // Billing Flow Interception State
  const [billingModeOpen, setBillingModeOpen] = useState(false);
  const [pendingBillingRoute, setPendingBillingRoute] = useState<string | null>(null);
  ...
  const startVoiceSearch = () => {
    ...
    recognition.lang = 'mr-IN'; // Hardcoded to Marathi dialect
  ...
  useEffect(() => {
    const searchDB = async () => {
      ...
      ).map(r => ({ name: r.label, path: r.href, icon: r.icon, type: 'route', onClick: r.onClick }));

      let orQueryString = `item_name.ilike.%${searchQuery}%,english_name.ilike.%${searchQuery}%`;
      
      // Intelligent Acronym / Initial Search
      if (searchQuery.length > 1 && searchQuery.length <= 6 && !searchQuery.includes(' ')) {
        const strictAcronymPattern = '%' + searchQuery.split('').join('%') + '%';       
        const spacedAcronymPattern = '%' + searchQuery.split('').join('% %') + '%';     
        
        orQueryString += `,item_name.ilike.${strictAcronymPattern},english_name.ilike.${strictAcronymPattern}`;
        orQueryString += `,item_name.ilike.${spacedAcronymPattern},english_name.ilike.${spacedAcronymPattern}`;
      }
  ...
  // Master Interceptor for Billing
  const handleNavigationClick = (route: string) => {
    if (route.startsWith("/billing")) {
      setPendingBillingRoute(route);
      setBillingModeOpen(true);
    } else {
      navigate(route);
    }
  };

  const handleBillingLaunch = (mode: string) => {
    setBillingModeOpen(false);
    const base = pendingBillingRoute || "/billing";
    const separator = base.includes("?") ? "&" : "?";
    navigate(`${base}${separator}mode=${mode}`);
    setPendingBillingRoute(null);
  };
  ...
  const SECONDARY_CATEGORIES = [
    "Pickles & Papad", "Tea & Coffee", "Salt & Essentials", "Essences & Colors",
    "Groceries & Ready Mixes", "Savai Masala", "Everest Masala", "Suhana / Pravin"
  ];

  const baseMenuOptions = [
    { href: "#", onClick: () => handleNavigationClick("/billing"), label: t('billing'), icon: ShoppingCart, description: "Start billing session", allowedRoles: ["admin", "manager", "sales"], iconBg: "bg-[#6366f1]" },
    { href: "/monthly-book", label: "Monthly List", icon: BookOpen, description: "Manage monthly grocery ledgers", allowedRoles: ["admin", "manager", "sales"], iconBg: "bg-[#8b5cf6]" },
  ...
  return (
    <AppLayout>
      <div className="min-h-screen bg-[#fcfcfd] font-sans pb-32">
        <div className="px-4 sm:px-6 max-w-5xl mx-auto pt-3 sm:pt-5 animate-in fade-in duration-300">
          ...
          <div className="relative mb-6 z-20">
            ...
            <button onClick={startVoiceSearch} className={`absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-xl transition-all ${isListening ? 'bg-rose-100 text-rose-600 animate-pulse' : 'bg-zinc-100 text-zinc-500 hover:bg-zinc-200 hover:text-zinc-700'}`}>
                    ...
                    {searchResults.map((result, idx) => (
                      <button 
                        ...
                        onClick={() => {
                          ...
                          else handleNavigationClick(result.path);
                        }}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-3">
              {QUICK_TILES.map((tile, idx) => (
                <button key={idx} onClick={() => handleNavigationClick(tile.path)} className="group relative overflow-hidden flex flex-col items-center justify-center p-4 rounded-[20px] bg-white border border-zinc-200/80 transition-all active:scale-95 shadow-sm hover:shadow-md hover:border-zinc-300">
            ...
            <div className="flex flex-wrap gap-2">
              {SECONDARY_CATEGORIES.map((cat, idx) => (
                <button key={idx} onClick={() => handleNavigationClick(`/billing?category=${encodeURIComponent(cat)}`)} className="flex items-center px-3 py-2 bg-white border border-zinc-200/80 rounded-[14px] text-xs font-bold text-zinc-600 shadow-sm hover:border-zinc-300 hover:shadow-md hover:text-zinc-900 active:scale-95 transition-all">
                  {cat} <ChevronRight className="h-3.5 w-3.5 ml-1 text-zinc-400" />
                </button>
              ))}
            </div>
          </div>

          <div className="mb-2.5"><h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.15em]">Your Workspace</h3></div>
            ...
            {visibleMenu.map((option, index) => {
              const content = (
                <Card className="h-full flex flex-col items-start justify-between p-3 sm:p-4 transition-all duration-200 rounded-[16px] border-zinc-200/80 bg-white shadow-sm hover:shadow-md hover:border-zinc-300 active:scale-95 cursor-pointer relative overflow-hidden">
                  ...
                  <div className={`h-9 w-9 rounded-[10px] flex items-center justify-center mb-2.5 shadow-sm ${option.iconBg} text-white shrink-0 relative z-10`}><option.icon className="h-4 w-4 sm:h-5 sm:w-5" /></div>
                  <div className="w-full relative z-10">
                    <h3 className="font-bold text-[12px] sm:text-[13px] tracking-tight text-zinc-900 leading-tight mb-0.5 truncate">{option.label}</h3>
                    <p className="text-[9px] sm:text-[10px] font-semibold text-zinc-500 line-clamp-2 leading-snug">{option.description}</p>
                  </div>
                </Card>
              );

              if (option.onClick) {
                return <button key={index} onClick={option.onClick} className="group outline-none block text-left h-full">{content}</button>;
              }

              return <button key={index} onClick={() => handleNavigationClick(option.href)} className="group outline-none text-left block h-full">{content}</button>;
            })}
          </div>
      ...
      {/* --- BILLING MODE SELECTION MODAL --- */}
      <Dialog open={billingModeOpen} onOpenChange={setBillingModeOpen}>
        <DialogContent aria-describedby={undefined} className="sm:max-w-md w-[95vw] rounded-[32px] p-6 text-center border-zinc-200 shadow-2xl bg-white">
          <DialogHeader>
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-indigo-50 border border-indigo-100 mb-2">
              <ShoppingCart className="h-8 w-8 text-indigo-500" />
            </div>
            <DialogTitle className="text-xl font-bold tracking-tight text-zinc-900">Select Billing Mode</DialogTitle>
            <DialogDescription className="text-sm font-medium text-zinc-500">How would you like to process this order?</DialogDescription>
          </DialogHeader>

          <div className="flex flex-col gap-3 mt-4">
            <Button onClick={() => handleBillingLaunch("walk_in")} variant="outline" className="h-16 justify-start px-5 rounded-2xl border-zinc-200 bg-white hover:bg-zinc-50 hover:border-zinc-300 transition-all text-left">
              <Store className="h-6 w-6 mr-4 text-emerald-500" />
              <div>
                <div className="font-bold text-[15px] text-zinc-900">Store Walk-in</div>
                <div className="text-[11px] font-semibold text-zinc-500">Standard counter checkout</div>
              </div>
            </Button>
            
            <Button onClick={() => handleBillingLaunch("monthly")} variant="outline" className="h-16 justify-start px-5 rounded-2xl border-zinc-200 bg-white hover:bg-zinc-50 hover:border-zinc-300 transition-all text-left">
              <BookOpen className="h-6 w-6 mr-4 text-purple-500" />
              <div>
                <div className="font-bold text-[15px] text-zinc-900">Monthly Customer</div>
                <div className="text-[11px] font-semibold text-zinc-500">Process against a monthly ledger</div>
              </div>
            </Button>
            
            <Button onClick={() => handleBillingLaunch("online")} variant="outline" className="h-16 justify-start px-5 rounded-2xl border-zinc-200 bg-white hover:bg-zinc-50 hover:border-zinc-300 transition-all text-left">
              <Globe className="h-6 w-6 mr-4 text-blue-500" />
              <div>
                <div className="font-bold text-[15px] text-zinc-900">Online Delivery</div>
                <div className="text-[11px] font-semibold text-zinc-500">WhatsApp / Call order for delivery</div>
              </div>
            </Button>
          </div>
        </DialogContent>
